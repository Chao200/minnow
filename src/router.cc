                        ...
                        const size_t interface_num )
{
  ...
  route_table_.push_back( RouteTable_(route_prefix, prefix_length_, 
                                      next_hop,interface_num) );
}
...
void Router::route() 
{
  for (auto& cur_interface : interfaces_)
  {
    ...
    while (datagram.has_value() && datagram->header.ttl>1)
    {
      ...
      {
        if (it->prefix_length_ == 0 || (dst>>(32 - it->prefix_length_))
      ...
      if ( match!=route_table_.end() )
      {
        ...
        // interface(match->interface_num_).send_datagram(*datagram, match->next_hop_.value_or( Address::from_ipv4_numeric( dst )));
      }
